...
export async function runInvestigation() {
  return request<InvestigationStatus & { job_id: string }>('/api/v1/investigate', {
    method: 'POST',
  }, 'tenant');
}

export async function getInvestigation(jobId: string) {
  return request<InvestigationStatus>(`/api/v1/investigate/${encodeURIComponent(jobId)}`, {}, 'tenant');
}
...
export async function downloadInvestigationResult(
  jobId: string,
  resultType: InvestigationResultType,
): Promise<{ ok: true; blob: Blob } | { ok: false; error: string }> {
  const key = getTenantKey();
  if (!key) return { ok: false, error: 'No tenant API key configured. Set it in Settings.' };

  try {
    const res = await fetch(
      `${BASE_URL}/api/v1/investigate/${encodeURIComponent(jobId)}/download/${resultType}`,
      { headers: { Authorization: `Bearer ${key}` } },
    );
    if (!res.ok) {
      ...
      try {
        const body = await res.json();
        error = body?.detail?.detail || body?.detail || body?.message || error;
      } catch { /* ignore non-JSON failure */ }
